...
def supervisor_view_attendance(request, id):
    ...
    if request.method == "GET":
        ...
        student.reg_num = student.reg_num.upper()
        # fin = ""
        # for l in reg:
        #     if l.isalpha():
        #         j = l.upper()
...
def supervisor_conf_attendance(request, id, std):
    ...
    if request.method == "GET":
        ...
        if decline and decline.lower() == "true":
            attendance.is_confirm = False
        else:
...
def student_dashboard(request):
    if request.method == "GET":
        ...
        now = timezone.now()
        today_start = timezone.make_aware(timezone.datetime(now.year, now.month, now.day, 0, 0, 0))
        today_end = timezone.make_aware(timezone.datetime(now.year, now.month, now.day, 23, 59, 59))

        # Check if the user has taken the first attendance today
        first_attendance_today = Attendance.objects.filter(student=data, date__range=(today_start, today_end)).first()

        # Determine if the user has taken the first attendance
        has_taken_first_attendance = True if first_attendance_today else False

        return render(request, "main/student/dashboard.html", {
            "data": data,
            "first_attendance": has_taken_first_attendance
        })
...
def student_attendance(request):
    if request.method == "POST":
        ...
        # Check if the user has taken attendance twice today
        attendance_count_today = Attendance.objects.filter(student=data, date__range=(today_start, today_end)).count()
        if attendance_count_today >= 2:
            return HttpResponse("You have already taken attendance twice today!")
